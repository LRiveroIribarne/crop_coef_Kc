...
setwd('D:/MEMORIA_LRI/HYDRUS_1D/salidas/MODELACION_DEFINITIVA_sin_invierno')

files = list.files(pattern = '.txt')
HYDRUS_out = list.files(path = paste0(getwd(), '/MODELACION_DEFINITIVA_sin_invierno'), pattern = '.out') 
...
Obs_Node = read.obs_node(project.path = 'D:/MEMORIA_LRI/HYDRUS_1D/salidas/MODELACION_DEFINITIVA_sin_invierno/MODELACION_DEFINITIVA_sin_invierno', out.file = "Obs_Node.out", obs.output = c('theta') ,obs.nodes = c(10,30,50,80) )

header = read.table(paste0(getwd(), '/MODELACION_DEFINITIVA_sin_invierno/', HYDRUS_out[9]), nrows = 1, header = FALSE, sep ='', stringsAsFactors = TRUE, skip = 5)
T_level = read.table(paste0(getwd(), '/MODELACION_DEFINITIVA_sin_invierno/', HYDRUS_out[9]), header = FALSE, sep = '', skip = 8, nrows = 658)
...
hum = data.frame(fecha = fecha, # Data frame con contenidos de agua HYDRUS y sonda
                 ...
                 hum80cm = Obs_Node$theta_80[1:length(fecha)],
                 ...
                 sonda80cm = sonda_pp_riego_ETo$SondaHum_80cm...)
...
for (i in 1:length(fecha)) {
  ...
  humedad = data.frame(prof = c(0,10,30,50,80,110), hum = c(hum = as.numeric(hum_sonda_transpuesta[2,i]), as.numeric(hum_sonda_transpuesta[2:5,i]), as.numeric(hum_sonda_transpuesta[5,i])))
  ApproxHum = approxfun(x = humedad$prof, y = humedad$hum)
  Prof = seq(0, 110, by = 1.1)
  LinearFitHum = ApproxHum(Prof)
  h_h20 = sum(LinearFitHum)
  WS_sonda = c(WS_sonda, h_h20)
...
g1 = ggplot(hum_melt10, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
...
g2 = ggplot(hum_melt30, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
...
g3 = ggplot(hum_melt50, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
...
#### REGRESION HYDRUS vs sonda ####

lm = lm(hum_val$sonda10cm ~ hum_val$hum10cm, data = hum_val)
gof = gof(hum_val$hum10cm, hum_val$sonda10cm)
MAPE = MAPE(hum_val$hum10cm, hum_val$sonda10cm)

g5 = ggplot(hum_val, aes(x = hum10cm ,y = sonda10cm)) +
  geom_point() + 
...
lm = lm(hum_val$sonda30cm ~ hum_val$hum30cm, data = hum_val)
gof = gof(hum_val$hum30cm, hum_val$sonda30cm)
MAPE = MAPE(hum_val$hum30cm, hum_val$sonda30cm)

g6 = ggplot(hum_val, aes(x = hum30cm ,y = sonda30cm)) +
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  geom_abline(intercept = lm$coefficients[1], slope = lm$coefficients[2]) +
  xlim(0.0,0.5) + 
...
lm = lm(hum_val$sonda50cm~hum_val$hum50cm, data = hum_val)
gof = gof(hum_val$hum50cm, hum_val$sonda50cm)
MAPE = MAPE(hum_val$hum50cm, hum_val$sonda50cm)

g7 = ggplot(hum_val, aes(x = hum50cm ,y = sonda50cm)) +
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  geom_abline(intercept = lm$coefficients[1], slope = lm$coefficients[2]) +
  xlim(0.0,0.5) + 
...
g9 = ggplot(TS_melt_1, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
...
g11 = ggplot(TS_val, aes(x = Ke_HYDRUS, y = Ke)) +
  ...
  annotate("text",x= 0, y=0.5, hjust = 0, label=paste("Slope =", round(lm$coefficients[2],2)))+
  annotate("text",x= 0, y=0.48, hjust = 0, label=paste("R2 =", round(gof[17],3))) +
  annotate("text",x= 0, y=0.46, hjust = 0, label=paste("PBIAS =", round(gof[6],3), '%')) +
  annotate("text",x= 0, y=0.44, hjust = 0, label=paste("RMSE =", round(gof[4],3))) +
  annotate("text",x= 0, y=0.42, hjust = 0, label=paste("NRMSE =", round(gof[5],3), '%')) +
  annotate("text",x= 0, y=0.40, hjust = 0, label=paste("MAPE =", round(MAPE,3))) +
  annotate("text",x= 0, y=0.38, hjust = 0, label=paste("NASH =", round(gof[9],3))) +
  theme_bw() + theme( panel.grid.major = element_blank(),
                      panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), aspect.ratio = 1)
...
g20 = ggplot(pp_melt[which(pp_melt$value>0),], aes(x = fecha, y = value)) +
  geom_bar(stat="identity",position = 'dodge', na.rm = TRUE, aes(fill = variable)) +
...
g23 = ggplot(WS_melt, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
...
gof = gof(WS_df_val$WS_sonda, WS_df_val$WS_HYDRUS)
MAPE = MAPE(WS_df_val$WS_sonda, WS_df_val$WS_HYDRUS)

g24 = ggplot(WS_df_val, aes(x = WS_HYDRUS , y = WS_sonda )) + geom_smooth(method="lm",formula = y ~ x , col = 'red', se = FALSE) +
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  geom_abline(intercept = lm$coefficients[1], slope = lm$coefficients[2]) +
  xlim(25,45) + 
  ylim(25,45) +
  ggtitle('WS HYDRUS vs WS sonda') +
  xlab('WS HYDRUS (cm)') + ylab('WS sonda (cm)') +
  annotate("text",x= 20, y=45, hjust = 0, label=paste("Slope =", round(lm$coefficients[2],2)))+
  annotate("text",x= 20, y=43, hjust = 0, label=paste("R2 =", round(gof[17],3))) +
  annotate("text",x= 20, y=41, hjust = 0, label=paste("PBIAS =", round(gof[6],3), '%')) +
  annotate("text",x= 20, y=39, hjust = 0, label=paste("RMSE =", round(gof[4],3))) +
  annotate("text",x= 20, y=37, hjust = 0, label=paste("NRMSE =", round(gof[5],3), '%')) +
  annotate("text",x= 20, y=35, hjust = 0, label=paste("MAPE =", round(MAPE,3))) +
  annotate("text",x= 20, y=33, hjust = 0, label=paste("NASH =", round(gof[9],3))) +
  theme_bw() + theme( panel.grid.major = element_blank(),
                      panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), aspect.ratio = 1)
...
WS_df$WS_HYDRUS_corr = WS_df$WS_HYDRUS *  1.0 + WS_df$WS_HYDRUS * gof[6]/100
WS_melt = reshape2::melt(WS_df,id.vars = c('fecha'), measure.vars = c('WS_sonda', 'WS_HYDRUS_corr'))

#write.csv(WS_df, 'WS_df.csv')

g25 = ggplot(WS_melt, aes(x = fecha, y = value, color = variable)) +
  geom_line(size = 1.0) + 
  ggtitle("Water storage") +
  xlab("Fecha") + ylab("water storage (cm)") +
  scale_x_date(breaks = '2 month', labels = fecha, date_labels = '%b %y') +
...
WS_df_val$WS_HYDRUS_corr = WS_df_val$WS_HYDRUS *  1.0 + WS_df_val$WS_HYDRUS * gof[6]/100


lm = lm(WS_df_val$WS_sonda ~ WS_df_val$WS_HYDRUS_corr, data = WS_df_val)
gof = gof(WS_df_val$WS_sonda, WS_df_val$WS_HYDRUS_corr)
MAPE = MAPE(WS_df_val$WS_sonda, WS_df_val$WS_HYDRUS_corr)

g26 = ggplot(WS_df_val, aes(x = WS_HYDRUS_corr , y = WS_sonda )) +
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  geom_abline(intercept = lm$coefficients[1], slope = lm$coefficients[2]) +
  xlim(25,40) + 
...
#### GROUPS
# hum_total = ggarrange(g1,g2,g3,g4, ncol = 2, nrow = 2)
# hum_reg = ggarrange(g5,g6,g7,g8, ncol = 2, nrow = 2)
# TS_total = g9
# Ke_TS_reg = ggarrange(g10, g11, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# Kcb_TS_reg = ggarrange(g12, g13, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# Kcb_pot_TS_reg = ggarrange(g14, g15, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# Kc_TS_reg = ggarrange(g16, g17, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# Kc_pot_TS_reg = ggarrange(g18, g19, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# pp = g20
# ETc_TS_reg = ggarrange(g21, g22, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# WS_TS = ggarrange(g23, g24, ncol = 3, nrow = 1, widths = c(3,1,0.05))
# WS_TS_bias_corr = ggarrange(g25, g26, ncol = 3, nrow = 1, widths = c(3,1,0.05))
...
# 
# g23 # WS TS
# g24 # WS HYDRUS_vs_WS_sonda
#
# g25 # WS TS bias correction
# g26 # WS HYDRUS_bias_corr_vs_WS_sonda
...
### individuales ####
 ...
 ggsave(paste0(getwd(),'/plots/4_hum_80cm_TS.jpg') ,plot= g4 ,width = 8,height=6,units="in",dpi=300)
 ...
 ggsave(paste0(getwd(),'/plots/8_hydrus_vs_sonda_80cm.jpg') ,plot= g8 ,width = 8,height=6,units="in",dpi=300)
 ...
 ggsave(paste0(getwd(),'/plots/24_WS_HYDRUSvs_WS_sonda.jpg') ,plot= g24 ,width = 8,height=6,units="in",dpi=300)
 ggsave(paste0(getwd(),'/plots/25_WS_bias_corr_TS.jpg') ,plot= g25 ,width = 8,height=6,units="in",dpi=300)
 ggsave(paste0(getwd(),'/plots/26_WS_HYDRUS_bias_corrvs_WS_sonda.jpg') ,plot= g26 ,width = 8,height=6,units="in",dpi=300)

 #### agrupados ####

 # ggsave(paste0(getwd(),'/plots/hum_total.jpg') ,plot= hum_total ,width = 8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/hum_reg.jpg') ,plot= hum_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/TS_total.jpg') ,plot= TS_total,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/Ke_TS_reg.jpg') ,plot= Ke_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/Kcb_TS_reg.jpg') ,plot= Kcb_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/Kcb_pot_TS_reg.jpg') ,plot= Kcb_pot_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/Kc_TS_reg.jpg') ,plot= Kc_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/Kc_pot_TS_reg.jpg') ,plot= Kc_pot_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/pp.jpg') ,plot= pp,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/ETc_TS_reg.jpg') ,plot= ETc_TS_reg,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/WS_TS.jpg') ,plot= WS_TS,width=8,height=6,units="in",dpi=300)
 # ggsave(paste0(getwd(),'/plots/WS_bias_corr_TS.jpg') ,plot= WS_TS_bias_corr,width=8,height=6,units="in",dpi=300)
